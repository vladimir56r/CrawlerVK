...
class Node:
    def __init__(self, node_name, label=None, shape="rectangle", font_family="Dialog",
                 ...
                 shape_fill="#FFFFFF", transparent="false", edge_color="#000000",
                 edge_type="line", edge_width="1.0", height=False, width=False, x=False,
                 y=False, img=None, border_has_color="true"):
        self.img = b64encode(img) if img else None
        self.label = label
        if label is None:
        ...
        self.border_has_color = border_has_color
    ...
    def convert(self):
        ...
        ET.SubElement(shape, "y:BorderStyle", color=self.edge_color, hasColor=self.border_has_color, type=self.edge_type,
...
class Edge:
    ...
    def convert(self):
        edge = ET.Element("edge", id=self.edge_id, source=str(self.node1), target=str(self.node2))
...
class Graph:
    ...
    def construct_graphml(self):
        ...
        res_key = ET.SubElement(graphml, "key", id="data_edge")
        res_key.set("for", "edge")
        res_key.set("yfiles.type", "edgegraphics")

        edge_key = ET.SubElement(graphml, "key", id="resources")
        edge_key.set("for", "graphml")
        edge_key.set("yfiles.type", "resources")

        graph = ET.SubElement(graphml, "graph", edgedefault=self.directed,
                              id=self.graph_id)
        res_data = ET.SubElement(graphml, "data", key="resources")
        resources = ET.SubElement(res_data, "y:Resources")
        for i, node_id in enumerate(self.nodes):
            node = self.nodes[node_id]
            node_xml = ET.Element("node", id=str(node.node_name))
            data = ET.SubElement(node_xml, "data", key="data_node")
            shape = ET.SubElement(data, "y:ShapeNode")
            if node.geom:
                ET.SubElement(shape, "y:Geometry", **node.geom)
            ET.SubElement(shape, "y:Fill", color=node.shape_fill,
                          transparent=node.transparent)
            ET.SubElement(shape, "y:BorderStyle", color=node.edge_color, type=node.edge_type,
                          width=node.edge_width, hasColor=node.border_has_color)
            label = ET.SubElement(shape, "y:NodeLabel", fontFamily=node.font_family,
                                  fontSize=node.font_size,
                                  underlinedText=node.underlined_text,
                                  fontStyle=node.font_style)
            
            if node.img:
                label.attrib["iconData"] = str(2 * (i + 1))
                label.attrib["iconTextGap"] = "4"
                label.text = node.label
                ET.SubElement(shape, "y:Shape", type=node.shape)
                res_descr = ET.SubElement(resources, "y:Resource", id=str(2 * (i + 1)))
                img_descr = ET.SubElement(res_descr, "yed:ScaledIcon", xScale="1.0", yScale="1.0")
                ET.SubElement(img_descr, "yed:ImageIcon", image=str(2 * (i + 1) + 1))
                img = ET.SubElement(resources, "y:Resource", id=str(2 * (i + 1) + 1), type="java.awt.image.BufferedImage")
                img.text = node.img.decode()
    ...
    def add_node(self, node_name, check_existance=True, **kwargs):
        if check_existance:
            if node_name in self.nodes.keys():
                raise RuntimeWarning("Node %s already exists" % node_name)
    ...
    def add_edge(self, node1, node2, label="", arrowhead="standard", arrowfoot="none",
                 ...
                 width="1.0", check_existance_nodes=True):
        # pass node names, not actual node objects
        if check_existance_nodes:
